...
// Test closely resembles the python test cisco torture.
...
void* delete_procedure (void* arg) {
	...
	while(loop) {
		...
		sprintf(query_string_buffer, "ALTER TABLE mt DROP TABLE pt_%d", part);
		printf("%s\n", query_string_buffer);
		if ((err = monetdbe_query(mdbe, query_string_buffer, NULL, NULL)) != NULL) {
			error(err)
			continue;
		...
		sprintf(query_string_buffer, "DROP TABLE pt_%d", part);
		printf("%s\n", query_string_buffer);
		if ((err = monetdbe_query(mdbe, query_string_buffer, NULL, NULL)) != NULL) {
			error(err)
			continue;
